...
const packagesRoot = path.join(root, 'cli');
...
function build(packagesToBuild, _opts, logger) {
    ...
    return Promise.resolve()
        ...
        .then(() => {
        const input = path.join(root, 'cli/lib/config/schema.json');
        const output = path.join(root, 'cli/lib/config/schema.d.ts');
        fs.writeFileSync(output, build_schema_1.buildSchema(input, logger), 'utf-8');
    })
        ...
        return files
            ...
            if (/(commands|tasks)[\\\/]_.+\.ts/.test(fileName)) {
                return true;
            }
        ...
        return !/[\\\/](@angular|ngs)[\\\/]cli[\\\/]blueprints/.test(fileName);
    }))
        .then(specFiles => {
        ...
        return Promise.all(extraFiles.map(fileName => {
            ...
            return copy(fileName, path.join('dist', fileName));
        }));
    })
        .then(() => {
...
//# sourceMappingURL=d:/fe.artisan/nsg/src/build.js.map